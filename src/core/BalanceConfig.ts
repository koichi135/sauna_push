...
export const BALANCE = {
  /**
   * ラン（1 プレイ）の構造。FB「目標が不明確」への対応。
   * 3 分のタイムアタックにし、時間切れ＝クリア（生存ボーナス）、体力 0＝失敗、
   * という明確な終わりと勝ち筋を作る。エンドレスの難度上昇 (BALANCE.sets) と
   * 組み合わせて「後半の熱波にどこまで耐えてハイスコアを伸ばすか」が主軸になる。
   */
  run: {
    durationSec: 180,
    /** 時間切れで終えたときのボーナス（体力割合を掛ける。満タンなら全額） */
    survivalBonus: 1500,
  },
   ...
   * ロウリュ。常設ボタン＋クールタイム制。
   *
   * FB「特に存在意義がわからない死に仕様」への対応で全面変更した。
   * 旧仕様は「濡らして数秒かけて温度が上がる」だけの受動効果で、投げ続ける方が
   * 単純に強く、使う理由が無かった。新仕様は狙った地点の山へ即座に衝撃を与えて
   * 手前へ押し出す**能動的な攻略ツール**にした（ヴィヒタの局所版）。
   * 押し出したストーンは 0.4 秒のコンボ窓に乗るので、「詰まった山をロウリュで
   * ほぐしてコンボにする」がプレイの中核になる。副次効果として軽く濡れて温度も上がる。
   */
  loyly: {
    /** 着弾点の影響半径 */
    radius: 0.13,
    /** 使用後のクールタイム (sec)。攻略ツールとして頻繁に使わせるため 12 → 6 に短縮 */
    cooldownSec: 6,
    /** ロウリュモードのまま放置した場合に解除されるまでの秒数 */
    modeTimeoutSec: 4.0,
    /** 手前方向に加える衝撃速度 (m/s)。ヴィヒタ (0.55) より狭い範囲に強く効かせる */
    pushVelocity: 0.5,
    /** わずかに持ち上げて摩擦から剥がす */
    liftVelocity: 0.12,
    /** ストーンが濡れている時間 (sec)。副次効果なので短め */
    wetDurationSec: 3.0,
    /** 濡れたストーン 1個あたりの毎秒温度上昇。副次効果なので控えめ */
    tempPerStonePerSec: 0.2,
    /** 温度上昇に数えるストーン数の上限 */
    maxStones: 8,
  ...
  /**
   * アイテム（仕様 8章 + 8.3）。ペイアウト口に落ちたときだけ発動する。
   *
   * FB「効果が弱い」「必ず時間で落ちてくるので取捨選択できない」への対応で、
   * オロポ以外の 3 種を**永続スタック型のレリック**に作り替えた（ローグライク要素）。
   * 一度きりの数秒バフではなく、拾うたびにビルドが強くなっていく。
   * 同時出現数を 1 個に絞り、間隔も伸ばして「今取りに行くか、盤面を崩さず
   * コンボを繋ぐか」という取捨選択が生まれるようにした（真の分岐選択はコインプッシャーの
   * 物理特性上アイテムを選べないため作れないが、出現時にトーストで種類を予告し、
   * 「狙うか無視するか」の判断材料にしている）。
   */
  items: {
    /** 前回出現からこの秒数が経てば無条件で出現。希少性を上げるため 32 → 50 に延長 */
    intervalSec: 50,
    /** 前回出現から minIntervalSec 経過 かつ payoutThreshold 回ペイアウトで出現 */
    minIntervalSec: 26,
    payoutThreshold: 24,
    /** 盤面上の同時存在数。複数出すと「取捨選択」が消えるので 1 個に絞る */
    maxOnBoard: 1,
    oropo: {
      staminaGain: 35,
    ...
    /**
     * ヴィヒタ: 拾った瞬間に盤面のストーン全部を手前へ一斉に扇ぐ（即時効果は維持）。
     * 加えて永続レリックとしてコンボ猶予をわずかに伸ばす（連鎖を作る腕前を後押しする）。
     */
    vihta: {
      pushVelocity: 0.55,
      liftVelocity: 0.12,
      /** 永続: コンボ猶予 +このぶん（秒）。スタックする */
      comboWindowBonusSec: 0.05,
    },
    /**
     * サウナハット: 永続レリック。スタックごとに温度帯ダメージを乗算で軽減する
     * （旧仕様は 8 秒だけの一時無効化で「弱い」と評価されていた）。
     */
    hat: {
      /** スタック 1 個あたりの倍率（0.85 = ダメージ 15% 軽減、乗算で重ねがけ） */
      damageMultPerStack: 0.85,
    },
    /**
     * 砂時計: 永続レリック。スタックごとに以後の外気浴（フィーバー）継続時間を延ばす。
     */
    hourglass: {
      /** 永続: フィーバー継続時間 +このぶん（秒）。スタックする */
      feverDurationBonusSec: 2.0,
    },
  },
  ...
  sets: {
     ...
     */
    // 3 分キャップに合わせて後半で明確に厳しくなるよう少し引き上げた
    levelPerMinute: 1.0,
